...
function get_posts(posts_id, posts_offset, time_delay) {
    $.ajax({
        url: 'https://api.vk.com/method/wall.get?owner_id=-' + posts_id + '&count=' + value_post+ '&offset=' + (posts_offset * value_post) + '&filter=owner&access_token=b6976c45b6976c45b6976c4562b6fb1a80bb697b6976c45ebc84db7bd66a7ecc589f3f8&v=5.101',
        type: 'GET',
        dataType: 'jsonp',
        crossDomain: true,
        success: function(data) {
			try {
				$('.main_left').empty();
				var posts = data.response.count >= value_post && posts_offset == 0 ? value_post : (data.response.count - (posts_offset * value_post));
				if (posts == 0) {
