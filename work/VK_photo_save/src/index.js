...
import { ActionSheet, ActionSheetItem, InfoRow, Progress, Placeholder, FixedLayout, Separator, CardGrid, Select, CellButton, Div, IOS, platform, Header, Card, CardScroll, Banner, Counter, Link, ScreenSpinner, View, Panel, PanelHeader, Group, Cell, PanelHeaderButton, Button, Avatar } from '@vkontakte/vkui';
...
class App extends React.Component {
	...
	Update(count, type, test) {
		...
		console.log(type);
		console.log(test);
		this.setState({ PhotoSelect: type.value });
		if (type.value !== "") {
			this.setState({ snackbar: null });
			this.setState({ popout: <ScreenSpinner /> });
			this.setState({ PhotoSelect: type.value });
			bridge.send("VKWebAppCallAPIMethod", {"method": "photos.get", "request_id": "photos.get", "params": {"owner_id":this.response.id, "album_id":type.value, "rev":1, "count":CountMin, "offset":(Count*CountMin), "v":version, "access_token":this.response.access_token}}).then(data => {
			...
			<View activePanel={this.state.activePanel} popout={this.state.popout}>
				...
				<Panel id="VKPhotoManager">
					...
					{this.response.PhotosGetAlbums &&
						<Group>
							<Div>
								<Select defaultValue={this.state.PhotoSelect} placeholder="Выберите альбом" name="album" onChange={this.Update.bind(this,'1')}>
									{this.response.PhotosGetAlbums.items.map(album =>
