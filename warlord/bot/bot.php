		...
		case "message_new":
			if($mess == $conf['mess']) {
				...
				curl_setopt_array($myCurl, array(
					CURLOPT_URL => $conf['apiurl'].'messages.send?user_id='.$u_id.'&group_id='.$conf['group_id'].'&attachment='.$photos_all[mt_rand(0, count($photos_all) - 1)].'&message='.urlencode('Пошёл нахуй держи фото').'&v='.$conf['v'].'&access_token='.$conf['standalone'],
